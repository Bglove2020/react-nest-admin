...
export class MenuService {
  ...
  async create(createMenuDto: CreateMenuDto) {
    ...
    if (createMenuDto.menuType !== 'M' && createMenuDto.parentPublicId) {
  ...
  async list(): Promise<FrontendMenu[]> {
    ...
    return buildTree<SysMenu, FrontendMenuBase>(menus, toFrontendDto);
  }
  ...
  async update(updateMenuDto: UpdateMenuDto) {
    ...
    try {
      menu = await this.menuRepository.findOne({
        where: { publicId: updateMenuDto.publicId },
      });
    } catch (e: any) {
      throw new BadRequestException({ msg: '数据库查询错误', code: 400 });
    }
    ...
    if (updateMenuDto.menuType !== 'M' && updateMenuDto.parentPublicId) {
      let parentMenu: SysMenu | null = null;
      try {
        parentMenu = await this.menuRepository.findOne({
  ...
  async delete(publicId: string) {
    ...
    try {
      subMenus = await this.menuRepository
        ...
        .where('menu.ancestors LIKE :ancestorId', {
          ancestorId: `%${menu.id}%`,
        })
        .getMany();
    } catch (e: any) {
